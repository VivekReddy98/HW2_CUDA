     ...
     * the wave equation in 2D */
     for( i = 0; i < n; i++)
     {
       for( j = 0; j < n; j++)
       {
         ...
         {
            NeighNeigh = 0.125*(uc[idx-2] + uc[idx+2] + uc[idx - 2*n] - uc[idx + 2*n]);  // WW, EE, NN, SS;
         }

         int immNeigh = 0.25*(uc[idx - n - 1] + uc[idx - n + 1] + uc[idx + n - 1] + uc[idx + n + 1]);  // NW, NE, SW, SE;

         int Neigh = uc[idx-1] + uc[idx+1] + uc[idx + n] + uc[idx - n]; // W, E, S, N
